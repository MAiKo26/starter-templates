# Hono + Bun Starter Template

A production-ready API starter built with **Hono**, **Bun**, **Drizzle ORM**, **Better Auth**, and **MinIO/S3**. Features full CRUD with OpenAPI docs, structured logging, rate limiting, and tests.

## Features

- **Hono** - Ultrafast web framework with Zod-based OpenAPI support
- **Bun** - Fast JavaScript runtime with native TypeScript execution
...
| `bun run build` | Build for production (Bun bundler) |
...
| `bun run fullcheck` | Lint + typecheck + test + format |
...
# Run container
docker run -p 8000:8000 --env-file .env hono-bun-app
```
